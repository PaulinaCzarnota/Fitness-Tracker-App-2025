...
class NotificationRepository @Inject constructor(
    ...
    fun getNotificationLogsByChannel(
        ...
        return notificationLogDao.getNotificationLogsByChannel(userId, channel)
    }
    ...
    fun getNotificationsByPriority(
        ...
        return notificationLogDao.getNotificationsByPriority(userId, priority)
    }
    ...
    suspend fun getMostCommonErrors(userId: Long, limit: Int = 10): List<ErrorFrequency> {
        return notificationLogDao.getMostCommonErrors(userId, limit)
    }
    ...
    suspend fun getInteractionRate(
        ...
        val stats = getNotificationDeliveryStats(userId, startDate, endDate)
        getNotificationLogsByDateRange(userId, startDate, endDate)
            .map { logs -> logs.count { it.eventType == NotificationEventType.CLICKED } }
            .let { flowOf(0) } // Placeholder - would need actual implementation

        return if (stats != null && stats.totalDelivered > 0) {
            // This would need proper implementation with actual clicked count
            0.0 // Placeholder
        } else {
            0.0
        }
     ...
     */
    suspend fun getNotificationInsights(userId: Long): NotificationAnalytics {
        val now = Date()
        val weekAgo = Date(now.time - (7 * 24 * 60 * 60 * 1000))
     ...
     */
    suspend fun getPerformanceMetrics(userId: Long): NotificationPerformanceMetrics {
        val now = Date()
        ...
        return NotificationPerformanceMetrics(
            ...
            maxRetryCount = maxRetries,
            topErrors = recentErrors.map { it.errorCode ?: "Unknown" },
    ...
    suspend fun cleanupOldLogs(retentionDays: Int = 90) {
        Date(System.currentTimeMillis() - (retentionDays * 24 * 60 * 60 * 1000L))
        // Implementation would require a new DAO method
        // notificationLogDao.deleteLogsOlderThan(cutoffDate)
    }
        ...
        getNotificationLogsByDateRange(userId, startDate, endDate)
            .map { logs ->
                if (includeSuccessful) {
                    ...
                    logs.filter { !it.isSuccessful }
                }
            }
        // This is a simplification - in real implementation would collect the Flow
