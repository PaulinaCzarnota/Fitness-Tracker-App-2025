...
class AppDatabaseTest {
    ...
    fun testWorkoutForeignKeyConstraint() = runTest {
        ...
        try {
            ...
            Assert.assertTrue("Exception should be foreign key related",
    ...
    fun testGoalProgressCalculation() = runTest {
        ...
        Assert.assertEquals("Progress should be 75%", 75.0f, 
            retrievedGoal?.getProgressPercentage(), 0.1f)
    ...
    fun testNotificationInsertAndRetrieve() = runTest {
        ...
        Assert.assertEquals("Type should match", NotificationType.WORKOUT_REMINDER, 
            retrievedNotification?.type)
        Assert.assertEquals("Title should match", "Workout Reminder", 
    ...
    fun testNotificationStatusUpdate() = runTest {
        ...
        Assert.assertEquals("Status should be SENT", NotificationStatus.SENT, 
    ...
    fun testFoodEntryInsertAndRetrieve() = runTest {
        ...
        Assert.assertTrue("Should contain the inserted food entry", 
    ...
    fun testFoodEntryNutritionCalculation() = runTest {
        ...
        Assert.assertEquals("Total calories should be correct", 210.0, 
            foodEntry.getTotalCalories(), 0.01)
        Assert.assertEquals("Total protein should be correct", 2.6, 
            foodEntry.getTotalProtein(), 0.01)
    }
    ...
    fun testMultipleEntityRelationships() = runTest {
        ...
        Assert.assertEquals("Notification should reference goal", goalId, 
    ...
    fun testInMemoryDatabaseCleanup() = runTest {
        ...
        Assert.assertTrue("User count should be > 0", 
        ...
        Assert.assertEquals("User count should be 0 after cleanup", 
            0, appDatabase.userDao().getUserCount())
    }

    @Test
    fun testDatabaseVersionAndMetadata() {
        // Test database version consistency
        Assert.assertEquals("Database version should match", 
            2, AppDatabase.getCurrentVersion())
